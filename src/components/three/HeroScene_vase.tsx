...
function ColumnMotif({ radius, y, count, height = 0.5 }: ColumnMotifProps) {
  return (
...
function BeadRing({ radius, y, count, beadSize = 0.08 }: BeadRingProps) {
  return (
    <group position={[0, y, 0]}>
      {Array.from({ length: count }).map((_, i) => {
        const angle = (i / count) * Math.PI * 2
  ...
  return (
    <DreiFloat speed={1.2} rotationIntensity={0.4} floatIntensity={0.3}>
      <group ref={meshRef} position={[0, -3.2, 0]} scale={[0.55, 0.55, 0.55]}>
        ...
        <BeadRing radius={0.75} y={0.3} count={20} beadSize={0.08} />

        {/* Architectural Body Band (Pillars pattern at the waist of the diamond body) */}
        <ColumnMotif radius={3.55} y={5.0} count={36} height={0.8} />
        ...
        <BeadRing radius={1.05} y={10.2} count={20} beadSize={0.08} />
        <ColumnMotif radius={1.05} y={10.3} count={20} height={0.5} />
        ...
        <BeadRing radius={2.9} y={12.3} count={40} beadSize={0.08} />
